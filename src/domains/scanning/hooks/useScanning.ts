...
import { useScanningStore } from '../store/useScanningStore.ts';
import type { ScanPhase } from '../models.ts';
import { useUIStore } from '../../../ui/store/useUIStore.ts';
import { useCardStore } from '../../card/store/useCardStore.ts';
import { useSentenceStore } from '../../sentence/store/useSentenceStore.ts';
import { useSituationStore } from '../../situation/store/useSituationStore.ts';
...
function getItemLabel(phase: ScanPhase, index: number): string {
  const { currentSlide } = useUIStore.getState();
  const { cards, currentCategory } = useCardStore.getState();
  const { activeSituation } = useSituationStore.getState();
...
export function useScanning() {
  ...
  const selectCategory = useCallback((index: number) => {
    ...
    if (currentSlide === 1) {
      ...
      const sitId = SITUATION_IDS[index];
      if (sitId) {
        useSituationStore.getState().setActiveSituation(sitId);
  ...
  const selectCard = useCallback((index: number) => {
    ...
    if (currentSlide === 1 && activeSituation) {
      ...
      const categoryCards = cardStore.cards[cardStore.currentCategory] || [];
      card = categoryCards[index - 1];
    }

    if (card) useSentenceStore.getState().selectCard(card);

    setTimeout(() => {
      const { currentMessage, selectedCards } = useSentenceStore.getState();
      if (currentMessage) {
        useUIStore.getState().openListenerModal(currentMessage, false, selectedCards);
  ...
  const selectModal = useCallback(() => {
    useUIStore.getState().closeListenerModal();
    useSentenceStore.getState().clearSelection();
    if ('speechSynthesis' in window) speechSynthesis.cancel();
  ...
  const returnToCategory = useCallback(() => {
    ...
    const { currentSlide } = useUIStore.getState();
    if (currentSlide === 1) {
      useSituationStore.getState().setActiveSituation(null);
    }
    enterCategoryPhase();
  ...
  const navigateScreen = useCallback(() => {
    ...
    if (ui.currentView !== 'app') {
      ui.setCurrentView('app');
    }

    const nextSlide = ((ui.currentSlide + 1) % SCREEN_COUNT) as SlideIndex;
    ui.setCurrentSlide(nextSlide);

    // 상황 화면에서 나가면 활성 상황 초기화
    if (ui.currentSlide === 1) {
      useSituationStore.getState().setActiveSituation(null);
    }
  ...
  const start = useCallback(() => {
    ...
    let slide = ui.currentSlide;
    if (!isScannableScreen(slide)) {
      slide = 0 as SlideIndex;
      ui.setCurrentSlide(slide);
    }
