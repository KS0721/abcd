 ...
 * - 없으면 → 자동 조사 삽입 (규칙 기반)
 *
 * 🤖 AI TODO: LLM 연결 시 이 함수를 아래와 같이 확장
 * ```
 * async function buildMessageWithAI(cards: Card[]): Promise<string> {
 *   const rawMessage = buildMessageRaw(cards);
 *   const response = await llm.complete({
 *     prompt: `다음 AAC 카드 조합을 자연스러운 한국어 문장으로 변환하세요: ${rawMessage}`,
 *     maxTokens: 100,
 *   });
